...
function inferBodyProfile(source=selected){
 ...
 if(/acura.*integra|integra.*type s/.test(text))return{style:'hatch',signature:'integra-type-s',w:1.9,l:4.72,body:.47,cabin:.58,cabinL:2.18,cabinZ:-.18,wheel:.4};
 ...
 if(/ac .*cobra|cobra gt|shelby cobra/.test(text))return{style:'roadster',signature:'cobra',w:1.75,l:3.95,body:.38,cabin:.24,cabinL:1.04,cabinZ:-.55,wheel:.42};
 if(/buick.*wildcat|wildcat/.test(text))return{style:'muscle',signature:'wildcat',w:1.96,l:5.15,body:.46,cabin:.48,cabinL:1.92,cabinZ:-.36,wheel:.43};
 ...
 if(/abarth.*punto/.test(text))return{style:'hatch',signature:'punto-evo',w:1.72,l:4.06,body:.5,cabin:.66,cabinL:2.08,cabinZ:-.08,wheel:.38};
 if(/abarth.*ritmo|ritmo/.test(text))return{style:'hatch',signature:'ritmo-130',w:1.67,l:3.96,body:.54,cabin:.7,cabinL:2.05,cabinZ:-.12,wheel:.36};
 ...
 if(/lamborghini|countach|aventador|hurac|revuelto/.test(text))return{style:'hypercar',signature:'lambo-wedge',w:2.04,l:4.58,body:.31,cabin:.42,cabinL:1.48,cabinZ:-.38,wheel:.41};
 if(/ferrari|mclaren|pagani|koenigsegg|ford gt|valkyrie|nevera/.test(text))return{style:'hypercar',signature:'exotic-low',w:2.02,l:4.62,body:.34,cabin:.46,cabinL:1.56,cabinZ:-.34,wheel:.41};
 if(/hypercar|supercar|chiron|veyron|corvette/.test(text))return{style:'hypercar',w:2.08,l:4.58,body:.34,cabin:.48,cabinL:1.62,cabinZ:-.35,wheel:.4};
...
function createCar(color=0xffa51f,playerCar=false,carSource=null){
 ...
 const addDetailBox=(x,y,z,w,h,d,mat=trim,rot=0)=>{const m=new THREE.Mesh(new THREE.BoxGeometry(w,h,d),mat);m.position.set(x,y,z);m.rotation.y=rot;car.add(m);return m};
 for(const sx of [-1,1]){
  addDetailBox(sx*(p.w/2+.035),ride+p.body*.46,.05,.035,.28,p.l*.5,trim);
  addDetailBox(sx*(p.w/2+.04),ride+.08,-.04,.055,.08,p.l*.72,trim);
  addDetailBox(sx*(p.w/2+.045),ride+p.body*.55,p.l*.18,.04,.38,.035,chrome);
  addDetailBox(sx*(p.w/2+.055),ride+p.body*.55,-p.l*.16,.04,.34,.035,chrome);
  addDetailBox(sx*(p.w/2+.11),ride+p.body+p.cabin*.2,p.cabinZ+p.cabinL*.28,.18,.1,.28,trim,sx*.08);
  addDetailBox(sx*(p.w/2+.045),ride+p.body*.25,p.l*.31,.06,.16,.72,paint);
  addDetailBox(sx*(p.w/2+.045),ride+p.body*.25,-p.l*.31,.06,.16,.72,paint);
 }
 if(p.style!=='roadster'){
  const windshield=new THREE.Mesh(new THREE.BoxGeometry(p.w*.58,.035,.44),glass);windshield.position.set(0,ride+p.body+p.cabin*.28,p.cabinZ+p.cabinL*.48);windshield.rotation.x=-.38;car.add(windshield);
  const rearWindow=new THREE.Mesh(new THREE.BoxGeometry(p.w*.54,.035,.42),glass);rearWindow.position.set(0,ride+p.body+p.cabin*.25,p.cabinZ-p.cabinL*.48);rearWindow.rotation.x=.35;car.add(rearWindow);
 }
 addDetailBox(0,ride+p.body+.055,p.l*.23,.035,.035,.88,trim);
 addDetailBox(0,ride+.08,p.l/2+.07,p.w*.9,.12,.16,trim);
 addDetailBox(0,ride+.11,-p.l/2-.07,p.w*.92,.15,.18,trim);
 ...
 if(p.signature==='integra-type-s'){const vent=new THREE.Mesh(new THREE.BoxGeometry(p.w*.34,.045,.58),dark);vent.position.set(0,ride+p.body+.08,p.l*.23);car.add(vent);const spoiler=new THREE.Mesh(new THREE.BoxGeometry(p.w*.82,.07,.28),dark);spoiler.position.set(0,ride+p.body+p.cabin*.42,-p.l/2+.12);car.add(spoiler);for(const sx of [-1,1]){const bronze=new THREE.MeshStandardMaterial({color:0xb7894e,metalness:.8,roughness:.24});const cap=new THREE.Mesh(new THREE.CylinderGeometry(p.wheel*.42,p.wheel*.42,.31,10),bronze);cap.rotation.z=Math.PI/2;cap.position.set(sx*(p.w/2+.021),ride+.08,p.l*.31);car.add(cap);const cap2=cap.clone();cap2.position.z=-p.l*.31;car.add(cap2)}}
 ...
 if(p.signature==='cobra'){const stripeMat=new THREE.MeshStandardMaterial({color:0xf4f4f4,roughness:.36});for(const x of [-.13,.13])addDetailBox(x,ride+p.body+.065,0,.08,.035,p.l*.92,stripeMat);const mouth=new THREE.Mesh(new THREE.BoxGeometry(p.w*.62,.18,.05),dark);mouth.position.set(0,ride+p.body*.35,p.l/2+.08);car.add(mouth);const roll=new THREE.Mesh(new THREE.TorusGeometry(.28,.035,8,18),chrome);roll.position.set(.34,ride+p.body+.28,-.8);roll.rotation.x=Math.PI/2;car.add(roll);for(const sx of [-1,1]){const pipe=new THREE.Mesh(new THREE.CylinderGeometry(.055,.055,p.l*.52,12),chrome);pipe.position.set(sx*(p.w/2+.12),ride+.16,-.18);pipe.rotation.x=Math.PI/2;car.add(pipe)}}
 if(p.signature==='wildcat'){const chromeStrip=new THREE.Mesh(new THREE.BoxGeometry(p.w*.96,.045,p.l*.78),chrome);chromeStrip.position.set(0,ride+p.body+.07,-.08);car.add(chromeStrip);const vinyl=new THREE.Mesh(new THREE.BoxGeometry(p.w*.7,.045,p.cabinL*.72),dark);vinyl.position.set(0,ride+p.body+p.cabin*.48,p.cabinZ);car.add(vinyl);for(const x of [-.52,-.24,.24,.52]){const lamp=new THREE.Mesh(new THREE.SphereGeometry(.11,12,8),new THREE.MeshStandardMaterial({color:0xf5f1d4,emissive:0xffe3a0,emissiveIntensity:2.5}));lamp.position.set(x,ride+p.body*.63,p.l/2+.06);lamp.scale.z=.3;car.add(lamp)}}
 if(p.signature==='punto-evo'||p.signature==='ritmo-130'){const stripeMat=new THREE.MeshStandardMaterial({color:0xc51422,roughness:.38});addDetailBox(0,ride+p.body+.06,-.08,p.w*.84,.035,.07,stripeMat);for(const sx of [-1,1])addDetailBox(sx*(p.w/2+.052),ride+p.body*.42,-.04,.035,.07,p.l*.65,stripeMat)}
 if(p.signature==='lambo-wedge'){const ventMat=new THREE.MeshStandardMaterial({color:0x050607,roughness:.48});for(const sx of [-1,1]){const intake=new THREE.Mesh(new THREE.BoxGeometry(.065,.32,.75),ventMat);intake.position.set(sx*(p.w/2+.06),ride+p.body*.35,-.2);intake.rotation.y=sx*.18;car.add(intake)}const wing=new THREE.Mesh(new THREE.BoxGeometry(p.w*.9,.065,.32),dark);wing.position.set(0,ride+p.body+.33,-p.l/2+.15);car.add(wing)}
 if(p.signature==='exotic-low'){for(const sx of [-1,1]){const intake=new THREE.Mesh(new THREE.BoxGeometry(.06,.22,.68),dark);intake.position.set(sx*(p.w/2+.055),ride+p.body*.34,.02);intake.rotation.y=sx*.12;car.add(intake)}const diffuser=new THREE.Mesh(new THREE.BoxGeometry(p.w*.82,.09,.22),dark);diffuser.position.set(0,ride+.08,-p.l/2-.14);car.add(diffuser)}
